...
import { 
  ...
  FileText, Settings, HelpCircle, AlertCircle
} from 'lucide-react';
    ...
    content: (
      <div className="space-y-8">
        <p className="text-xl font-light leading-relaxed">
          ...
          global knowledge indexes using local LLM synthesis.
        </p>
        <div className="grid md:grid-cols-2 gap-6">
          <div className="p-6 border border-my-border bg-white/50 dark:bg-slate-900/50">
          ...
          <div className="p-6 border border-my-border bg-white/50 dark:bg-slate-900/50">
             <h4 className="font-bold uppercase tracking-widest text-xs text-my-accent mb-4">Local Execution</h4>
             <p className="text-sm opacity-80 leading-relaxed">Your data never leaves your hardware. By utilizing Ollama and Llama 3.2, every research session is private by design.</p>
          </div>
        </div>
      </div>
    )
        ...
        <div className="space-y-6">
           ...
           <div className="flex gap-6 items-start">
              ...
              <div>
                 ...
                 <p className="text-sm opacity-70">The system executes parallel crawls of global indexes. The local Llama model then cross-references findings to eliminate noise and detect contradictions.</p>
              </div>
           </div>
           <div className="flex gap-6 items-start">
  ...
  },
  {
    ...
    content: (
      <div className="space-y-8">
        ...
        <div className="p-8 bg-my-ink text-white space-y-6">
           <h4 className="text-2xl font-serif italic border-b border-white/10 pb-4">Protocol Requirements</h4>
        ...
        <div className="p-6 border border-dashed border-my-accent/30 text-center">
           <p className="text-sm italic opacity-60">Deep Research can take 2-3 minutes depending on your hardware's VRAM capacity.</p>
        </div>
      </div>
    )
  },
    ...
    content: (
      <div className="space-y-8">
        <div className="flex items-center gap-4 p-4 bg-emerald-500/10 border border-emerald-500/20 text-emerald-600 dark:text-emerald-400">
           <Lock size={20} />
           <span className="font-bold uppercase tracking-widest text-xs">Security Status: Air-Gapped Optimized</span>
        </div>
        ...
        <div className="space-y-4">
           ...
           <div className="p-6 border border-my-border">
              <h5 className="font-bold mb-2 flex items-center gap-2"><Database size={16} /> The Intelligence Vault</h5>
              <p className="text-sm opacity-70">Your data is stored in a local SQLite vault (`cognapse.db`). This ensures that your investigative history survives browser refreshes and remains logically partitioned from other operatives on the same machine.</p>
           </div>
        </div>
      </div>
    )
                ...
                <button
                  ...
                  className={clsx(
                    ...
                    activeSection === section.id 
                      ? "bg-my-ink text-white dark:bg-white dark:text-my-ink shadow-xl translate-x-2" 
