...
export async function generatePremiumPDF({ query, report, deepThesis, aiProvider }: PDFGeneratorInput): Promise<void> {
  ...
  // Professional style helpers (Light Theme)
  const cardStyle = "background-color: #FFFFFF; border: 1px solid #E2E8F0; padding: 35px; margin-bottom: 30px; border-radius: 8px; box-sizing: border-box; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.05);";
  const sectionTitleStyle = "color: #0F172A; font-size: 18px; text-transform: uppercase; letter-spacing: 0.05em; margin-top: 0; margin-bottom: 24px; font-weight: 800; border-bottom: 2px solid #2563EB; padding-bottom: 12px; line-height: 1.2;";
  const highlightBoxStyle = "background-color: #F8FAFC; border-left: 4px solid #3B82F6; padding: 20px; margin-bottom: 24px; border-radius: 0 8px 8px 0;";
  const tableHeaderStyle = "padding: 12px 16px; background-color: #F1F5F9; color: #334155; font-weight: 700; text-transform: uppercase; font-size: 11px; border-bottom: 2px solid #CBD5E1;";
  const tableCellStyle = "padding: 14px 16px; color: #475569; font-size: 12px; border-bottom: 1px solid #E2E8F0; line-height: 1.5;";
  ...
  // 1. COVER PAGE
  let coverPage = `
    <div style="height: 1000px; display: flex; flex-direction: column; justify-content: space-between; padding: 40px; box-sizing: border-box; background: white;">
      
      <!-- Top Section: Logo & Brand -->
      <div style="border-bottom: 1px solid #E2E8F0; padding-bottom: 30px;">
        <h1 style="color: #0F172A; font-size: 42px; font-family: 'Inter', sans-serif; font-weight: 900; margin: 0; letter-spacing: -0.02em;">COGNAPSE</h1>
        <p style="font-size: 14px; text-transform: uppercase; letter-spacing: 0.2em; color: #64748B; margin: 8px 0 0 0; font-weight: 600;">Research & Intelligence Platform</p>
      </div>

      <!-- Middle Section: Report Title -->
      <div style="flex-grow: 1; display: flex; flex-direction: column; justify-content: center;">
        <span style="font-size: 12px; font-weight: 700; text-transform: uppercase; color: #2563EB; letter-spacing: 0.15em; margin-bottom: 16px; display: inline-block;">Comprehensive Analysis Report</span>
        <h2 style="font-size: 48px; font-family: 'Playfair Display', serif; font-weight: 700; color: #0F172A; margin: 0 0 24px 0; line-height: 1.15;">${query}</h2>
        <div style="width: 80px; height: 6px; background-color: #2563EB; border-radius: 3px;"></div>
      </div>

      <!-- Bottom Section: Metadata -->
      <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 40px; border-top: 1px solid #E2E8F0; padding-top: 30px; font-family: 'Inter', sans-serif; background: #F8FAFC; padding: 30px; border-radius: 8px;">
        <div>
          <div style="margin-bottom: 16px;">
            <span style="display: block; font-size: 10px; text-transform: uppercase; color: #64748B; font-weight: 700; letter-spacing: 0.1em; margin-bottom: 4px;">Report ID</span>
            <span style="font-size: 14px; color: #0F172A; font-family: monospace; font-weight: 600;">${reportId}</span>
          </div>
          <div>
            <span style="display: block; font-size: 10px; text-transform: uppercase; color: #64748B; font-weight: 700; letter-spacing: 0.1em; margin-bottom: 4px;">Generation Date</span>
            <span style="font-size: 14px; color: #0F172A; font-weight: 500;">${dateStr}</span>
          </div>
        </div>
        <div>
          <div style="margin-bottom: 16px;">
            <span style="display: block; font-size: 10px; text-transform: uppercase; color: #64748B; font-weight: 700; letter-spacing: 0.1em; margin-bottom: 4px;">AI Providers</span>
            <span style="font-size: 14px; color: #0F172A; font-weight: 500;">${aiProvider.toUpperCase()} Multi-Model Ensemble</span>
          </div>
          <div>
            <span style="display: block; font-size: 10px; text-transform: uppercase; color: #64748B; font-weight: 700; letter-spacing: 0.1em; margin-bottom: 4px;">Export Version</span>
            <span style="font-size: 14px; color: #0F172A; font-weight: 500;">v${exportVersion}</span>
          </div>
        </div>
      </div>
    </div>
  ...
  if (report) {
    ...
    let tocHTML = `
      <div style="${cardStyle}">
        ...
        <div style="font-family: 'Inter', sans-serif; font-size: 14px; color: #334155; line-height: 2.2;">
          ...
          <div style="display: flex; justify-content: space-between; border-bottom: 1px dotted #CBD5E1; margin-bottom: 8px;">
            <span style="font-weight: 600;">5. References Section</span>
          </div>
          <div style="display: flex; justify-content: space-between; border-bottom: 1px dotted #CBD5E1; margin-bottom: 8px;">
            <span style="font-weight: 600;">6. Future Research Directions</span>
          </div>
          <div style="display: flex; justify-content: space-between; border-bottom: 1px dotted #CBD5E1; margin-bottom: 8px;">
            <span style="font-weight: 600;">7. Appendix & Report Metadata</span>
          </div>
        </div>
      </div>
    `;
    ...
    let execHTML = `
      <div style="${cardStyle}">
        ...
        <div style="${highlightBoxStyle}">
          <span style="font-size: 10px; font-weight: 800; text-transform: uppercase; color: #1D4ED8; letter-spacing: 0.1em; display: block; margin-bottom: 12px;">Overall Conclusion</span>
          <p style="font-size: 15px; line-height: 1.6; color: #0F172A; margin: 0; font-weight: 500; font-style: italic;">
            "${safeText(report.summary.bottom_line)}"
          </p>
        </div>

        <div style="margin-bottom: 24px;">
          <h4 style="color: #0F172A; font-size: 14px; font-weight: 700; margin: 0 0 12px 0;">Major Insights & Important Observations</h4>
          <p style="font-size: 13px; line-height: 1.7; color: #475569; margin: 0; text-align: justify;">${safeText(report.summary.full_synthesis)}</p>
        </div>

        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 30px;">
          <div>
            <h4 style="color: #0F172A; font-size: 12px; text-transform: uppercase; margin: 0 0 12px 0; font-weight: 700; letter-spacing: 0.05em;">Key Findings</h4>
            <ul style="margin: 0; padding-left: 20px; font-size: 12px; color: #475569;">
              ${formatList(premium.executive_summary.key_findings)}
            </ul>
          </div>
          <div>
            <h4 style="color: #0F172A; font-size: 12px; text-transform: uppercase; margin: 0 0 12px 0; font-weight: 700; letter-spacing: 0.05em;">Critical Insights</h4>
            <ul style="margin: 0; padding-left: 20px; font-size: 12px; color: #475569;">
    ...
    let consensusHTML = `
      <div style="${cardStyle}">
        ...
        <div style="display: flex; gap: 30px; align-items: stretch; margin-bottom: 30px;">
          <!-- Visual Element: Score -->
          <div style="flex-shrink: 0; background: #F8FAFC; border: 1px solid #E2E8F0; padding: 30px; border-radius: 8px; width: 160px; display: flex; flex-direction: column; justify-content: center; align-items: center; box-shadow: inset 0 2px 4px rgba(0,0,0,0.02);">
            <div style="font-size: 48px; font-weight: 900; color: #10B981; line-height: 1;">${premium.multi_ai_consensus.consensus_score}%</div>
            <div style="font-size: 10px; text-transform: uppercase; color: #64748B; letter-spacing: 0.1em; font-weight: 700; margin-top: 12px; text-align: center;">Overall Consensus Score</div>
          </div>
          
          <div style="flex-grow: 1; display: flex; flex-direction: column; gap: 16px;">
            <div style="background: #ECFDF5; border-left: 4px solid #10B981; padding: 16px; border-radius: 0 4px 4px 0;">
              <span style="font-size: 11px; font-weight: 800; text-transform: uppercase; color: #047857; display: block; margin-bottom: 8px; letter-spacing: 0.05em;">Agreement Between Systems</span>
              <ul style="margin: 0; padding-left: 20px; font-size: 12px; color: #064E3B; line-height: 1.5;">
                ${formatList(premium.multi_ai_consensus.agreement_points)}
              </ul>
            </div>
            
            <div style="background: #FEF2F2; border-left: 4px solid #EF4444; padding: 16px; border-radius: 0 4px 4px 0;">
              <span style="font-size: 11px; font-weight: 800; text-transform: uppercase; color: #B91C1C; display: block; margin-bottom: 8px; letter-spacing: 0.05em;">Differing Viewpoints</span>
        ...
        <div style="margin-top: 24px;">
          <h4 style="color: #0F172A; font-size: 13px; font-weight: 700; margin: 0 0 16px 0;">Confidence Indicators & Model Breakdown</h4>
    ...
    let advancedHTML = `
      <div style="${cardStyle}">
        ...
        <div style="margin-bottom: 24px;">
          <h4 style="color: #0F172A; font-size: 14px; font-weight: 700; margin: 0 0 12px 0;">Deeper Synthesis</h4>
          <p style="font-size: 13px; line-height: 1.7; color: #475569; margin: 0; text-align: justify;">${safeText(premium.advanced_analysis.deeper_synthesis)}</p>
        </div>
        
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 24px; margin-bottom: 30px;">
          <div style="background: #F8FAFC; border: 1px solid #E2E8F0; padding: 20px; border-radius: 6px;">
            <h4 style="color: #0F172A; font-size: 12px; text-transform: uppercase; margin: 0 0 12px 0; font-weight: 700;">Expanded Reasoning</h4>
            <p style="font-size: 12px; line-height: 1.6; color: #475569; margin: 0;">${safeText(premium.advanced_analysis.expanded_reasoning)}</p>
          </div>
          <div style="background: #F8FAFC; border: 1px solid #E2E8F0; padding: 20px; border-radius: 6px;">
            <h4 style="color: #0F172A; font-size: 12px; text-transform: uppercase; margin: 0 0 12px 0; font-weight: 700;">Contradiction Analysis</h4>
            <p style="font-size: 12px; line-height: 1.6; color: #475569; margin: 0;">${safeText(premium.advanced_analysis.contradiction_analysis)}</p>
          </div>
        </div>

        <div style="${highlightBoxStyle}">
          <span style="font-size: 10px; font-weight: 800; text-transform: uppercase; color: #1D4ED8; letter-spacing: 0.1em; display: block; margin-bottom: 8px;">Strategic Interpretation & Recommendations</span>
          <p style="font-size: 13px; line-height: 1.6; color: #0F172A; margin: 0; font-weight: 500;">${safeText(premium.advanced_analysis.strategic_interpretation)}</p>
        </div>
      </div>
    `;
    ...
    if (report.timeline_events && report.timeline_events.length > 0) {
      let timelineHTML = `
        <div style="${cardStyle}">
          ...
          <div style="position: relative; padding-left: 30px; border-left: 2px solid #E2E8F0; margin-left: 10px; margin-top: 20px;">
            ${report.timeline_events.map(event => `
              <div style="margin-bottom: 24px; position: relative;">
                <div style="position: absolute; left: -37px; top: 4px; width: 12px; height: 12px; border-radius: 50%; background-color: #2563EB; border: 3px solid #FFFFFF; box-shadow: 0 0 0 1px #E2E8F0;"></div>
                <div style="display: flex; align-items: baseline; gap: 12px; margin-bottom: 6px;">
                  <span style="font-family: 'Inter', sans-serif; font-size: 11px; font-weight: 700; color: #2563EB; background: #EFF6FF; padding: 4px 8px; border-radius: 4px;">${safeText(event.date)}</span>
                  <span style="font-size: 10px; font-weight: 600; text-transform: uppercase; color: #64748B;">Impact: ${event.significance || 9}/10</span>
                </div>
                <h4 style="font-size: 14px; color: #0F172A; font-weight: 700; margin: 0 0 6px 0;">${safeText(event.title)}</h4>
                <p style="font-size: 12px; color: #475569; margin: 0; line-height: 1.6;">${safeText(event.description)}</p>
              </div>
            `).join('')}
          </div>
        </div>
      ...
      let highlightsHTML = `
        <div style="${cardStyle}">
          ...
          <div style="display: grid; grid-template-columns: 1fr; gap: 20px; margin-bottom: 24px;">
            <div style="background: #F0FDF4; border: 1px solid #BBF7D0; padding: 20px; border-radius: 8px; border-left: 4px solid #22C55E;">
              <span style="font-size: 10px; font-weight: 800; text-transform: uppercase; color: #166534; display: block; margin-bottom: 8px; letter-spacing: 0.1em;">Important Insight</span>
              <p style="font-size: 13px; margin: 0; color: #14532D; line-height: 1.6; font-weight: 500;">${safeText(take.key_insight)}</p>
            </div>
            
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
              <div style="background: #FEF2F2; border: 1px solid #FECACA; padding: 20px; border-radius: 8px; border-left: 4px solid #EF4444;">
                <span style="font-size: 10px; font-weight: 800; text-transform: uppercase; color: #991B1B; display: block; margin-bottom: 8px; letter-spacing: 0.1em;">Risk Indicator</span>
                <p style="font-size: 13px; margin: 0; color: #7F1D1D; line-height: 1.6;">${safeText(take.watch_out_for)}</p>
              </div>
              <div style="background: #EFF6FF; border: 1px solid #BFDBFE; padding: 20px; border-radius: 8px; border-left: 4px solid #3B82F6;">
                <span style="font-size: 10px; font-weight: 800; text-transform: uppercase; color: #1E40AF; display: block; margin-bottom: 8px; letter-spacing: 0.1em;">Recommendation</span>
                <p style="font-size: 13px; margin: 0; color: #1E3A8A; line-height: 1.6;">${safeText(take.next_step)}</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <table style="width: 100%; border-collapse: collapse; text-align: left; border: 1px solid #E2E8F0; border-radius: 4px; overflow: hidden;">
            ...
            <tbody>
              ${report.sources.map(s => `
                <tr>
                  <td style="${tableCellStyle}; max-width: 300px;">
                    <div style="font-weight: 600; color: #0F172A; margin-bottom: 4px;">${safeText(s.title)}</div>
                    ${s.url && s.url !== "URL unavailable" && !s.url.includes("unavailable") ? `
                      <div style="font-size: 10px; color: #2563EB; word-break: break-all;">${safeText(s.url)}</div>
                    ` : ''}
                  </td>
                  <td style="${tableCellStyle}; font-weight: 500;">${safeText(s.domain || "Web Resource")}</td>
                  <td style="${tableCellStyle}; text-align: center; font-weight: 700; color: #10B981;">${s.relevance_score || 90}%</td>
                </tr>
              `).join('')}
            </tbody>
          </table>
    ...
    let futureHTML = `
      <div style="${cardStyle}">
        <h3 style="${sectionTitleStyle}">7. Future Research Directions</h3>
        <p style="font-size: 13px; color: #475569; margin-bottom: 16px; line-height: 1.6;">Based on the findings and limitations of the current analysis, the following areas are recommended for continuation topics and deeper investigation:</p>
    ...
    let appendixHTML = `
      <div style="${cardStyle}">
        <h3 style="${sectionTitleStyle}">8. Appendix & Report Metadata</h3>
        
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 24px; margin-bottom: 30px;">
          <div style="background: #F8FAFC; border: 1px solid #E2E8F0; padding: 20px; border-radius: 6px;">
            <h4 style="color: #64748B; font-size: 11px; text-transform: uppercase; margin: 0 0 12px 0; font-weight: 700; letter-spacing: 0.1em;">Processing Details</h4>
            <div style="display: flex; flex-direction: column; gap: 8px; font-size: 12px; color: #0F172A;">
              <div style="display: flex; justify-content: space-between; border-bottom: 1px solid #E2E8F0; padding-bottom: 4px;">
                <span style="color: #475569;">Synthesis Depth</span>
                <strong>${premium.metadata.synthesis_depth}/10</strong>
              </div>
              <div style="display: flex; justify-content: space-between; border-bottom: 1px solid #E2E8F0; padding-bottom: 4px;">
                <span style="color: #475569;">Research Complexity</span>
                <strong>${premium.metadata.research_complexity}/10</strong>
              </div>
              <div style="display: flex; justify-content: space-between; padding-bottom: 4px;">
                <span style="color: #475569;">Routing</span>
                <strong>${safeText(premium.metadata.model_routing)}</strong>
              </div>
            </div>
          </div>
          
          <div style="background: #F8FAFC; border: 1px solid #E2E8F0; padding: 20px; border-radius: 6px;">
            <h4 style="color: #64748B; font-size: 11px; text-transform: uppercase; margin: 0 0 12px 0; font-weight: 700; letter-spacing: 0.1em;">Document Information</h4>
            <div style="display: flex; flex-direction: column; gap: 8px; font-size: 12px; color: #0F172A;">
              <div style="display: flex; justify-content: space-between; border-bottom: 1px solid #E2E8F0; padding-bottom: 4px;">
              ...
              <div style="display: flex; justify-content: space-between; padding-bottom: 4px;">
                <span style="color: #475569;">Report ID</span>
  ...
  sandboxStyle.innerHTML = `
    @import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800;900&family=Playfair+Display:ital,wght@0,600;0,700;1,700&display=swap');
